...
<script type="text/javascript">
	...
	<section>
		<div id="background-container">
			<div id="image-background" style="background-image: url({$Setting.custom_login_background_img}); background-size: cover; background-position: center top;"></div>
			<video id="video-background" autoplay="" loop="" muted="" playsinline="" src="{$Setting.custom_login_background_img}" style="object-fit: cover; object-position: center top; display: block;"></video>
			<style>
				...
				<div class="col-md-7 col-lg-6 col-xl-5 py-6 py-md-0">
					<div class="card shadow zindex-100 mb-0 overflow-hidden rounded-lg semi-transparent-white-bg">
						<div class="p-1 bg-info d-flex text-white justify-content-center">
							<strong>{$Setting.custom_login_background_char ? $Setting.custom_login_background_char : $Setting.company_name}</strong>
						</div>
						<div class="card-body px-md-5 py-5">
							...
							<form id="loginForm" class="mb-0" method="post" action="/login?action=email">
							 {if $Login.allow_login_wechat}
                                <div id="wechat" class="tab-pane  {if $Login.allow_login_wechat == 1 || !$Get.action}active{/if}" role="tabpanel">
                                    {php}$hooks=hook('wechat_login');{/php}
                                    {if $hooks}
						...
						<div class="p-1 bg-info d-flex flex-column text-white">
							...
							<span class="text-center">
								登录即代表同意 <a href="{$Setting.web_privacy_url}" target="_blank" class="text-light">《服务协议》</a> <a href="{$Setting.web_tos_url}" target="_blank" class="text-light">《用户隐私条款》</a>
							</span>
						</div>
					</div>
				</div>
